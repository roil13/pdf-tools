...
| `npm run android:apk` | The above, then assembles a sideloadable **debug** APK into `android/app/build/outputs/apk/debug/`. Debug rather than release because there is no signing config — an unsigned release APK will not install — and because a debug build stays reachable from `chrome://inspect`, which is how the camera capture gets diagnosed on real hardware. |
